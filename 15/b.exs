defmodule MatrixPath do
  ...
  # Utility to replicate the matrix 5x5 with incrementing digits and collapse into one 2D matrix
  def replicate_and_collapse_matrix_5x5(matrix) do
    orig_rows = length(matrix)
    orig_cols = length(List.first(matrix))

    # Generate the collapsed 2D matrix
    for row_factor <- 0..4, row <- 0..(orig_rows - 1) do
      for col_factor <- 0..4, col <- 0..(orig_cols - 1) do
        # Calculate the new value, wrapping around using modulo 9
        original_value = get_value(matrix, {row, col})
        new_value = rem(original_value + row_factor + col_factor - 1, 9) + 1
        new_value
      end
    end
    |> Enum.chunk_every(orig_cols * 5)
  end
end
...
matrix = MatrixPath.replicate_and_collapse_matrix_5x5(matrix)
shortest_sum = MatrixPath.shortest_path_sum(Enum.at(matrix, 0))
